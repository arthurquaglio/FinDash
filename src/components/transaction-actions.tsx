...
export function TransactionActions({ transaction, types, categories }: any) {
        ...
        <div className="flex justify-end gap-2">
            ...
            <Sheet open={isUpdateOpen} onOpenChange={setIsUpdateOpen}>
                ...
                <SheetContent className="bg-zinc-900 border-zinc-800 text-zinc-100">
                    <SheetHeader>
                        <SheetTitle className="bg-sm text-zinc-400" >Editar Transação</SheetTitle>
                    </SheetHeader>
                    <form
                    ...
                    >
                        ...
                        <select name="typeId" defaultValue={transaction.typeId} className="bg-zinc-950 border-zinc-800 rounded-md h-10 px-2 text-sm">
                            {types.map((t: any) => <option key={t.id} value={t.id}>{t.name}</option>)}
                        </select>

                        <select name="categoryId" defaultValue={transaction.categoryId} className="bg-zinc-950 border-zinc-800 rounded-md h-10 px-2 text-sm">
